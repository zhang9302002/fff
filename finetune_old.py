...
from models.vstream_qwen2vl import FlashVStreamQwen2VLModel
...
"""
{
    ...
    "videos": [
        "data_link/eval_video/videomme/video_frames/fFjv93ACGo8"
    ]
},
...
def preprocess(
    ...
    for i, source in enumerate(sources):
        ...
        if 'feature' in video_path:
            feature_path = video_path[:-4] + '.safetensors'
            data = load_file(feature_path)
            video_emb = data['image_embeds']
            video_grid_thw_0 = data['video_grid_thw'][0]
            assert video_grid_thw_0[1:].prod() * 14 * 14 <= data_args.max_pixels, f"{video_path} is too large, video_grid_thw={video_grid_thw_0} > {data_args.max_pixels}"
            if video_grid_thw_0[0] * 2 > data_args.max_frames:
                max_grid_t = data_args.max_frames // 2
                hidden_dim = video_emb.shape[-1]
                video_emb = video_emb.reshape(video_grid_thw_0[0], video_grid_thw_0[1] // 2, video_grid_thw_0[2] // 2, -1)
                idx = torch.linspace(0, video_emb.shape[0] - 1, max_grid_t).round().long()
                video_emb = video_emb[idx]
                # print(f'feature too large, resize from {video_grid_thw_0} to {video_emb.shape}')
                video_grid_thw_0 = torch.tensor([video_emb.shape[0], video_emb.shape[1] * 2, video_emb.shape[2] * 2])
                video_emb = video_emb.reshape(-1, hidden_dim)

            video_embeds.append(video_emb)
        elif 'frame' in video_path:
            ...
            video_inputs = processor.image_processor(images=None, videos=video_inputs, return_tensors="pt")
            ...
            if idx % 2 == 0:
                ...
                if is_first_flag:  # first query, with vision embedding
                    ...
                    _input_id = im_start + tokenizer('user').input_ids + nl_tokens + \
                                tokenizer('<|vision_start|>').input_ids + tokenizer('<|video_pad|>').input_ids * (video_grid_thw_0.prod() // 4) + tokenizer('<|vision_end|>').input_ids + \
                                tokenizer(query).input_ids + im_end + nl_tokens
                    input_id += _input_id
                    target += im_start + [IGNORE_TOKEN_ID] * (len(_input_id) - 3) + im_end + nl_tokens
                    visual_position_id += [-1] * len(im_start + tokenizer('user').input_ids + nl_tokens + \
                                tokenizer('<|vision_start|>').input_ids) + [x for x in range(video_grid_thw_0.prod() // 4)] + [-1] * len(tokenizer('<|vision_end|>').input_ids + \
                                tokenizer(query).input_ids + im_end + nl_tokens)
                else:  # others not
                    _input_id = im_start + tokenizer('user').input_ids + nl_tokens + \
                                tokenizer(query).input_ids + im_end + nl_tokens
                ...
                input_id += _input_id
                target += im_start + [IGNORE_TOKEN_ID] * len(tokenizer('assistant').input_ids) + \
                                        _input_id[len(tokenizer('assistant').input_ids) + 1:-2] + im_end + nl_tokens
...
class LazySupervisedDataset(Dataset):
    ...
    def __init__(self, raw_data, tokenizer: transformers.PreTrainedTokenizer, processor, max_len: int, data_args):
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        ...
        ret = preprocess([self.raw_data[i]["conversations"]], [self.raw_data[i]['video']], self.tokenizer, self.processor, self.max_len, self.data_args)
        ret2 = dict(
        ...
        elif len(ret["video_embeds"]) > 0:
            ret2["video_embeds"] = ret["video_embeds"][0]
            # print(f'LazySupervisedDataset[{i}] contains, video_embeds={ret2["video_embeds"].shape}')
...
def make_supervised_data_module(
    tokenizer: transformers.PreTrainedTokenizer, processor, data_args, max_len,
) -> Dict:
    ...
    train_dataset = dataset_cls(train_json, tokenizer=tokenizer, processor=processor, max_len=max_len, data_args=data_args)
...
def train():
    ...
    data_module = make_supervised_data_module(
        tokenizer=tokenizer, processor=processor, data_args=data_args, max_len=training_args.model_max_length
    )
