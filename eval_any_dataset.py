...
def launch_multi_gpu_eval(args, dataset_name, frame_dir, data_file, evaluation_name='evaluation'):
    ...
    if not args.test:
        if 'videochatgpt' in dataset_name:
            ...
            # test_i = 2
            # split_list = split_list[test_i: test_i+1]
            # data_file_list = data_file_list[test_i: test_i+1]
        else:
            split_list = [""]
            data_file_list = [data_file]
        for data_file, split in zip(data_file_list, split_list):
            ...
            for idx in range(0, num_chunks):
                cmd = [ "python3", "inference_ovo_vqa.py" if dataset_name in ['ovobench', 'streambench'] else "inference_mcq_vqa.py",
...
def get_dataset_info(args):
    ...
    dataset_list = [
        {'type': 'mc', 'dataset_name': 'egoschema', 'frame_dir': './data_link/eval_video/EgoSchema/frames', 'data_file': './data_link/eval_video/EgoSchema/test_qa.json'},
        {'type': 'mc', 'dataset_name': 'egoschema_all', 'frame_dir': './data_link/eval_video/EgoSchema/frames', 'data_file': './data_link/eval_video/EgoSchema/all_qa.json'},
        {'type': 'mc', 'dataset_name': 'videommesub', 'frame_dir': './data_link/eval_video/videomme/frames', 'data_file': './data_link/eval_video/videomme/test_qa.json'},
        {'type': 'mc', 'dataset_name': 'videommewo', 'frame_dir': './data_link/eval_video/videomme/frames', 'data_file': './data_link/eval_video/videomme/test_qa.json'},
        {'type': 'mc', 'dataset_name': 'mvbench', 'frame_dir': './data_link/eval_video/mvbench/frames', 'data_file': './data_link/eval_video/mvbench/test_qa.json'},
        {'type': 'mc', 'dataset_name': 'lvbench', 'frame_dir': './data_link/eval_video/lvbench/frames', 'data_file': './data_link/eval_video/lvbench/test_qa.json'},
        {'type': 'mc', 'dataset_name': 'mlvu', 'frame_dir': './data_link/eval_video/mlvu/frames', 'data_file': './data_link/eval_video/mlvu/test_qa.json'},
        {'type': 'oe', 'dataset_name': 'rvs_ego', 'frame_dir': './data_link/eval_video/vstream-realtime/ego4d_frames', 'data_file': './data_link/eval_video/vstream-realtime/test_qa_ego4d.json'},
        {'type': 'oe', 'dataset_name': 'rvs_movie', 'frame_dir': './data_link/eval_video/vstream-realtime/movienet_frames', 'data_file': './data_link/eval_video/vstream-realtime/test_qa_movienet.json'},
        {'type': 'oe', 'dataset_name': 'actnet', 'frame_dir': './data_link/eval_video/ActivityNet-QA/test_frames', 'data_file': './data_link/eval_video/ActivityNet-QA/test_qa.json'},
        {'type': 'oe', 'dataset_name': 'nextoe', 'frame_dir': './data_link/eval_video/nextoe/nextoe_frames', 'data_file': './data_link/eval_video/nextoe/test_qa.json'},
        {'type': 'mc', 'dataset_name': 'ovobench', 'frame_dir': './data_link/eval_video/ovobench/frames', 'data_file': './data_link/eval_video/ovobench/test_qa.json'},
        {'type': 'oe', 'dataset_name': 'streambench', 'frame_dir': './data_link/eval_video/streambench/frames', 'data_file': './data_link/eval_video/streambench/test_qa.json'},
    ]
    dataset_list.append({'type':'oe', 
        'dataset_name': f'videochatgpt', 
        'frame_dir': './data_link/eval_video/VideoChatGPTBench/video_10000frames_high_fps1',
        'data_file': ['./data_link/eval_video/VideoChatGPTBench/test_{split}_qa.json'.format(split=split) for split in ["generic", "temporal", "consistency"]]})
...
def calc_eval_result(output_path, num_chunks, data_path):
    ...
    for sample in pred_contents:
        if 'ovobench' in output_path and sample['a_type'] in ["REC", "SSR", "CRR"]:
            def get_score_REC(response):
                if response == None:
                    return 0
                response = re.findall(r'\d+', response)
                response = "".join(response)
                return int(response)
            if sample['a_type'] == "REC":
                res = get_score_REC(sample['pred'])
            else:
                y_matches = re.findall('yes', sample['pred'], re.IGNORECASE)
                n_matches = re.findall('no', sample['pred'], re.IGNORECASE)
                if len(y_matches) > len(n_matches):
                    res = 1
                else:
                    res = 0
        else:
    ...
    if 'ovobench' in output_path:
        realtime_tasks = ["OCR", "ACR", "ATR", "STU", "FPD", "OJR", "realtime_avg"]
        backward_tasks = ["EPM", "ASI", "HLD", "backward_avg"]
        forward_tasks = ["REC", "SSR", "CRR", "forward_avg"]
        all_tasks = realtime_tasks + backward_tasks + forward_tasks
        for task in all_tasks:
            if task not in meter_dic:
                meter_dic[task] = ScoreMeter()
        ...
        if 'a_type' in result and result['a_type'] is not None:
            ...
            if 'next' in output_path:
                ...
                if typ not in meter_dic:
                    meter_dic[typ] = ScoreMeter()
                meter_dic[typ].add_score(score, acc)
            elif 'ovobench' in output_path:
                if typ in realtime_tasks:
                    typ = "realtime_avg"
                elif typ in backward_tasks:
                    typ = "backward_avg"
                elif typ in forward_tasks:
                    typ = "forward_avg"
                meter_dic[typ].add_score(score, acc)
...
def calc_gpt_based_eval_result(args, output_path, num_chunks, data_path):
    ...
    if 'videochatgpt' in output_path:
        ...
        code_list = [
            "data_link/eval_video/VideoChatGPTBench/evaluate_benchmark_1_correctness.py",
            "data_link/eval_video/VideoChatGPTBench/evaluate_benchmark_2_detailed_orientation.py",
            "data_link/eval_video/VideoChatGPTBench/evaluate_benchmark_3_context.py",
            "data_link/eval_video/VideoChatGPTBench/evaluate_benchmark_4_temporal.py",
            "data_link/eval_video/VideoChatGPTBench/evaluate_benchmark_5_consistency.py",
        ]
        ...
        for split, code, short in zip(split_list, code_list, short_list):
            ...
            exec(cmd)
    elif 'streambench' in output_path:
        # mkdir ${SAVE_PATH}/StreamingBench_v0.3
        # for IDX in $(seq 0 $((${ngpus}-1))); do
        #     CUDA_VISIBLE_DEVICES=${IDX} python3 eval_streambench_with_llama3.py \
        #         --predict_file ${SAVE_PATH}/evaluation_${suffix}/streambench \
        #         --output_dir ${SAVE_PATH}/StreamingBench_v0.3 \
        #         --output_name ${ngpus}_${IDX} \
        #         --llama3_path ckpt/llama3-8B-Instruct \
        #         --num_chunks ${ngpus} \
        #         --chunk_idx ${IDX} &
        # done
        eval_path = os.path.join(output_path, "StreamingBench_v0.3")
        if not args.testtest:
            for idx in range(num_chunks):
                name = f"{num_chunks}_{idx}"
                if num_chunks == 1:
                    name = "pred"
                cmd = ["python", "eval_streambench_with_llama3.py",
                        "--predict_file", output_path,
