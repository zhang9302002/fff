...
class ModelArguments:
    ...
    flash_memory_temporal_method: str = 'sample'
    flash_memory_temporal_poolsize: int = 1
    flash_memory_spatial_length: int = 0
    flash_memory_spatial_method: str = 'sample'
...
def train():
    ...
    # 只解冻 model.visual.merger 模块的参数
    for param in model.visual.merger.parameters():
        param.requires_grad = True
    
    trainable_params = [name for name, param in model.named_parameters() if param.requires_grad]
    print("[main] Trainable parameters:", trainable_params)
