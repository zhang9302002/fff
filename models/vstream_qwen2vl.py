...
class FlashMemory(nn.Module):
    ...
    def temporal_compress(self, x, thw, temporal_length):
        ...
        if self.temporal_method in method_dic:
            x, weights, centroids_timestamp, step_indices = method_dic[self.temporal_method](x, temporal_length)
        else:
    ...
    def spatial_enhance(self, x, small_x, thw, tem_x, tem_thw, tem_centroids_timestamp, centroids, weights, step_indices):
        ...
        print(f'Perform spatial_enhance, method = {self.spatial_method}, x.shape={x.shape}, spa_x.shape={spa_x.shape}, tem_x.shape={tem_x.shape}, cat_x.shape={cat_x.shape}')
        return cat_x, spa_positions, tem_positions
        ...
        for x, small_x, thw, small_thw in zip(x_list, small_x_list, grid_thw, small_grid_thw):
            ...
            if self.spatial_length > 0:
                if self.spatial_method in ['klarge_retrieve', 'nearest', 'klarge_retrieve_cos']:
                    x, spa_positions, tem_positions = self.spatial_enhance(
                        x=x, 
                        small_x=small_x,
                        thw=thw, 
