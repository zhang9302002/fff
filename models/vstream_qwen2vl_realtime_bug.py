"""
Develop Log
10.26: update forward(), load from safetensor ViT feature, instead of image or videos.
10.26: update, use deepspeed-ulysses, failed
"""
...
def get_pooled_thw(thw, pool_size):
    if pool_size <= 1:
        return thw
    assert pool_size == 2, f"Only support pool_size=2, pool_size={pool_size}"
    t, h, w = thw
    h = h // 2
    w = w // 2
    if h % 2 != 0:
...
def get_pooled_compressed_thw(thw, pool_size, compress_length):
    t, h, w = get_pooled_thw(thw, pool_size)
    t = min(t, compress_length)
    return [t, h, w]
...
class FlashMemory(nn.Module):
    ...
    def temporal_compress(self, x, thw, temporal_length, weights=None, times=None):
        # grid_thw is [T/2, H/14, W/14]
        # x.shape is [grid_t x grid_h/2 x grid_w/2 x 2 x 2, 1280]
        t, h, w = thw
        if t <= temporal_length:
            return x, torch.ones(t, device=x.device), torch.arange(t, device=x.device, dtype=torch.int32), [[i] for i in range(t)]
        assert h % 2 == 0
        assert w % 2 == 0
        x = x.reshape(t, h // 2 * w // 2 * 2 * 2, x.shape[-1])
        if temporal_length == 0:
            x = x[:0, ...]
            return x.reshape(-1, x.shape[-1]), torch.ones(0, device=x.device), torch.arange(0, device=x.device, dtype=torch.int32), []
        ...
        if self.temporal_method in method_dic:
            method_fn = method_dic[self.temporal_method]
        else:
            raise ValueError(f"temporal_method should be one of {method_dic.keys()}")
        if weights is not None:
            method_fn = partial(method_fn, weights=weights)
        if times is not None:
            method_fn = partial(method_fn, times=times)
        x, weights, centroids_timestamp, step_indices = method_fn(x, temporal_length)
        return x.reshape(-1, x.shape[-1]), weights, centroids_timestamp, step_indices

    def merge_spa_tem(self, spa_x, tem_x):
        xdim = spa_x.shape[-1]
    ...
    def spatial_enhance(self, x, small_x, thw, tem_x, tem_thw, tem_centroids_timestamp, centroids, weights, step_indices):
        t, h, w = thw
        xdim = x.shape[-1]
        ...
        tem_positions = tem_centroids_timestamp.round().long()
        # assert weights.shape[0] == st, f"weights.shape={weights.shape} [0] should be equal to st={st}"  # sometimes is less than st
        method_list = ['sample', 'klarge', 'klarge_retrieve']
        if t <= self.spatial_length:
            ...
            if self.spatial_method == 'sample':
                ...
                spa_x = x[idx]
                spa_positions = idx
            elif self.spatial_method == 'klarge':
                print(f'weights={weights.shape} {weights}')
                sorted_indices = torch.argsort(weights, descending=True)
                klarge_indices = sorted_indices[:self.spatial_length]
                klarge_cluster_centroids = torch.zeros_like(x[:self.spatial_length])
                for i, idx in enumerate(klarge_indices):
                    w = weights[idx]
                    indices = step_indices[idx]
                    if len(indices) > 0:
                        indices = torch.tensor(indices, device=x.device, dtype=torch.int32)
                        klarge_cluster_centroids[i] = torch.mean(x[indices], dim=0)
                    else:
                        raise ValueError(f"klarge_indices, i={i} has no indices, step_indices={step_indices[i]}")
                dist = ((klarge_cluster_centroids.reshape(self.spatial_length, 1, -1) - x.reshape(1, t, -1)) ** 2).sum(dim=-1)  # [k, t]
                idx = torch.argmin(dist, dim=1)
                spa_x = x[idx]
                spa_positions = idx
                # print(f'Perform spatial_enhance, klarge, klarge_indices={klarge_indices}, weights={weights[klarge_indices]}, step_indices={step_indices}, spa_position={spa_positions}')
            elif self.spatial_method == 'klarge_retrieve':  # calc dist by small x
                centroids = centroids.reshape(st, sh // 2 * sw // 2 * 2 * 2, xdim)
                sorted_indices = torch.argsort(weights, descending=True)
                klarge_indices = sorted_indices[:self.spatial_length]
                centroids = centroids[klarge_indices].reshape(self.spatial_length, -1) # [T1, P*D]
                dist = ((centroids.reshape(self.spatial_length, 1, -1) - small_x.reshape(1, t, -1)) ** 2).sum(dim=-1)  # [k, t]
                idx = torch.argmin(dist, dim=1)
                spa_x = x[idx]
                spa_positions = idx
            elif self.spatial_method == 'nearest':
                sorted_indices = torch.argsort(weights, descending=True)
                klarge_indices = sorted_indices[:self.spatial_length]
                spa_positions = tem_positions[klarge_indices]
                spa_x = x[spa_positions]
            elif self.spatial_method == 'klarge_retrieve_cos':
                centroids = centroids.reshape(st, sh // 2 * sw // 2 * 2 * 2, xdim)
                # print(f'>>> Cos calculation 1, centroids={centroids.shape}, small_x={small_x.shape}, x={x.shape}, temp_x={tem_x.shape}')
                sorted_indices = torch.argsort(weights, descending=True)
                klarge_indices = sorted_indices[:self.spatial_length]
                # print(f'>>> Cos calculation 1.5, sorted_indices={sorted_indices.shape}, klarge_indices={klarge_indices.shape}')
                centroids = centroids[klarge_indices].reshape(self.spatial_length, -1) # [T1, P*D]
                centroids = centroids.reshape(self.spatial_length, -1) # [T1, P*D]
                # print(f'>>> Cos calculation 2, centroids={centroids.shape}, small_x={small_x.shape}, x={x.shape}, temp_x={tem_x.shape}, klarge_indices={klarge_indices}')
                small_x = small_x.reshape(t, -1) # [t, P*D]
                centroids_norm = centroids / centroids.norm(dim=-1, keepdim=True)  # [k, d]
                small_x_norm = small_x / small_x.norm(dim=-1, keepdim=True)  # [t, d]
                cosine_similarity = torch.matmul(centroids_norm, small_x_norm.transpose(0, 1))  # [k, t]
                spa_positions = torch.argmin(cosine_similarity, dim=1)
                spa_x = x[spa_positions]
            else:
                raise ValueError(f"spatial_method should be one of {method_list}")
        print(f'Perform spatial_enhance, method = {self.spatial_method}, x.shape={x.shape}, spa_x.shape={spa_x.shape}')
        return spa_x, spa_positions, tem_positions
    ...
    def forward(self, x, grid_thw, small_grid_thw, position_ids, visual_position_ids):
        ...
        new_x = []
        for x, small_x, thw, small_thw in zip(x_list, small_x_list, grid_thw, small_grid_thw):
            small_compressed_x, tem_weights, centroids_timestamp, tem_indices = self.temporal_compress(small_x, small_thw, self.temporal_length)
            if self.spatial_length > 0:
                if self.spatial_method in ['klarge_retrieve', 'nearest', 'klarge_retrieve_cos']:
                    spa_x, spa_positions, tem_positions = self.spatial_enhance(
                        x=x, 
                        small_x=small_x,
                        thw=thw, 
                        tem_x=small_compressed_x, 
                        tem_thw=get_pooled_compressed_thw(thw, self.temporal_poolsize, self.temporal_length), 
                        tem_centroids_timestamp=centroids_timestamp,
                        centroids=small_compressed_x,
                        weights=tem_weights, 
                        step_indices=tem_indices,
                    )
                else:
                    centroids, weights, _, step_indices = self.temporal_compress(small_x, small_thw, self.spatial_length)
                    spa_x, spa_positions, tem_positions = self.spatial_enhance(
                        x=x, 
                        small_x=small_x,
                        thw=thw, 
                ...
                x = self.merge_spa_tem(spa_x=spa_x, tem_x=small_compressed_x)
            else:
                x = small_compressed_x
                spa_positions = torch.tensor([], device=x.device).long()
                tem_positions = centroids_timestamp.round().long()
            new_x.append(x)
        x = torch.stack(new_x, dim=0)

        """ Position_ids update only suppor batchsize=1 now!"""
        mask = visual_position_ids[0] >= 0
        true_indices = torch.nonzero(mask, as_tuple=False)
        visual_start_pos = true_indices[0].item()
        visual_start_id = position_ids[0, 0, visual_start_pos]  # only suppor batchsize=1
        assert position_ids[0, 0, visual_start_pos] == position_ids[1, 0, visual_start_pos]
        assert position_ids[1, 0, visual_start_pos] == position_ids[2, 0, visual_start_pos]
        visual_end_pos = true_indices[-1].item()
        ...
        spa_pos_ids, spa_size = get_mm_index_with_positions(get_spatial_real_grid_thw(grid_thw[0], self.config), spa_positions)
        tem_pos_ids, tem_size = get_mm_index_with_positions(get_real_grid_thw(grid_thw[0], self.config), tem_positions)
        ...
        position_ids[:, 0, mask] = flash_memory_position_ids  # [3, videoL]
...
class FlashVStreamQwen2VisionTransformerPretrainedModel(Qwen2VLPreTrainedModel):
    ...
    def forward_simple_not_merge(self, hidden_states: torch.Tensor, grid_thw: torch.Tensor) -> torch.Tensor:
...
class FlashVStreamQwen2VLModel(Qwen2VLForConditionalGeneration):
    ...
    def embed_new_video_clip(self, pixel_values_videos, video_grid_thw, idx):
        def merge_thw(thw_1, thw_2):
            assert thw_1[1:] == thw_2[1:]
            res = thw_1.clone()
            res[0] += thw_2[0]
            return res
        ...
        dummy_position_ids = torch.zeros((1, video_grid_thw.prod()), dtype=torch.long, device=self.device)
        pixel_values_videos = pixel_values_videos.type(self.visual.get_dtype())
        video_embeds, _ = self.visual(pixel_values_videos, grid_thw=video_grid_thw, position_ids=dummy_position_ids, visual_position_ids=visual_position_ids)
# self.visual
        x = self.visual.forward_simple_not_merge(pixel_values_videos, video_grid_thw)
        small_pixel_values_videos, small_thw = self.visual.flash_memory.temporal_pool(x, video_grid_thw[0])
        small_x = self.visual.forward_simple_not_merge(small_pixel_values_videos, small_thw.unsqueeze(0))
# self.visual.flash_memory
        # init
        tem_x = small_x
        thw = video_grid_thw[0]
        # tem_thw = get_pooled_compressed_thw(thw, flash.temporal_poolsize, flash.temporal_length)
        tem_weights = torch.ones(x.size(0), dtype=x.dtype, device=x.device)
        centroids_timestamp = torch.arange(x.size(0), dtype=x.dtype, device=x.device)
        # merge
        if self.video_embedding_memory is not None and len(self.video_embedding_memory) > 0:
            cuda_list = []
            for item in self.video_embedding_memory:
                cuda_list.append(item.cuda())
            old_tem_x, old_tem_thw, old_tem_weights, old_centroids_timestamp, \
            old_spa_x, old_thw = cuda_list
            tem_x = torch.cat([old_tem_x, tem_x], dim=0)
            small_thw = merge_thw(old_tem_thw, small_thw)
            tem_weights = torch.cat([old_tem_weights, tem_weights], dim=0)
            centroids_timestamp = torch.cat([old_centroids_timestamp, centroids_timestamp], dim=0)
            spa_x = torch.cat([old_spa_x, spa_x], dim=0)
            thw = merge_thw(old_thw, thw)
        

        flash = self.visual.flash_memory
        small_compressed_x, tem_weights, centroids_timestamp, tem_indices = flash.temporal_compress(
            tem_x, small_thw, flash.temporal_length, tem_weights, centroids_timestamp
        )
        
        assert self.spatial_length > 0
        assert self.spatial_method in ['klarge_retrieve', 'nearest', 'klarge_retrieve_cos']
        spa_x, spa_positions, tem_positions = flash.spatial_enhance(
            x=x, 
            small_x=tem_x,
            thw=thw, 
            tem_x=small_compressed_x, 
        ...
        with self.video_embedding_mem_lock:
            self.video_embedding_memory[:] = [
                tem_x.cpu(), tem_thw.cpu(), tem_weights.cpu(), centroids_timestamp.cpu(),
                spa_x.cpu(), thw.cpu(), 
            ]  # Only change content
            logger.info(f'Writevideo_embeds={video_embeds.shape} {video_embeds.dtype}')

        return []

    def prepare_realtime_inference(self, position_ids):
        assert self.use_video_streaming_mode
        logger = logging.getLogger(__name__)
        attempt_times = 0
        while attempt_times < 300:
            try:
                with self.video_embedding_mem_lock:
                    video_embeds, = self.video_embedding_memory
                    logger.info(f'Read video_embeds={video_embeds.shape} {video_embeds.dtype}')
        ...
        video_embeds = self.visual.merger(memory)
        return video_embeds, position_ids
    ...
    def forward(
        ...
        if self.use_video_streaming_mode:
            video_embeds, position_ids = self.prepare_realtime_inference(position_ids)
            video_embeds = video_embeds.to(inputs_embeds.device)  # [N, dim]
            video_mask = input_ids == self.config.video_token_id
            inputs_embeds[video_mask] = video_embeds
