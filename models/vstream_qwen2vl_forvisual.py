...
class FlashMemory(nn.Module):
    ...
    def spatial_enhance(self, x, thw, tem_x, tem_thw, weights, centroids_timestamp, step_indices, meta):
        t, h, w = thw
        xdim = x.shape[-1]
        ...
        print(f'Perform spatial_enhance, x.shape={x.shape}, spa_x.shape={spa_x.shape}, tem_x.shape={tem_x.shape}, cat_x.shape={cat_x.shape}')
    ...
    def forward(self, x, grid_thw, small_grid_thw, position_ids, visual_position_ids):
        ...
        for x, small_x, thw, small_thw in zip(x_list, small_x_list, grid_thw, small_grid_thw):
            small_compressed_x, _, centroids_timestamp, meta['temporal_step_indices'] = self.temporal_compress(small_x, small_thw, self.temporal_length)
            meta['small_x'] = small_x
            meta['x'] = x
            if self.spatial_length > 0:
                _, weights, _, step_indices = self.temporal_compress(small_x, small_thw, self.spatial_length)
                meta['spatial_step_indices'] = step_indices
                x, spa_positions, tem_positions = self.spatial_enhance(
                    ...
                    weights=weights, 
                    centroids_timestamp=centroids_timestamp,
...
class FlashVStreamQwen2VisionTransformerPretrainedModel(Qwen2VLPreTrainedModel):
    ...
    def forward(
        ...
        hidden_states, position_ids, meta = self.flash_memory(hidden_states, grid_thw, small_grid_thw, position_ids, visual_position_ids)
        hidden_states = self.merger(hidden_states)
        return hidden_states, position_ids, meta
...
class FlashVStreamQwen2VLModel(Qwen2VLForConditionalGeneration):
    ...
    def forward(
        ...
        if inputs_embeds is None:
            ...
            print(f'inputs_embeds={inputs_embeds.dtype}')
            # load from image
            ...
            if pixel_values_videos is not None:
                ...
                print(f'pixel_values_videos={pixel_values_videos.dtype}')
                video_embeds, position_ids, meta = self.visual(pixel_values_videos, grid_thw=video_grid_thw, position_ids=position_ids, visual_position_ids=visual_position_ids)
    ...
    def prepare_inputs_for_generation(
        ...
        **kwargs,
        ...
        # input_ids = input_ids.to(self.model.embed_tokens.weight.dtype)
        # pixel_values_videos = pixel_values_videos.to(self.model.embed_tokens.weight.dtype)
