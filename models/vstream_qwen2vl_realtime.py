...
class FlashMemory(nn.Module):
    ...
    def temporal_compress(self, x, thw, temporal_length, temporal_weights, temporal_indices):
        ...
        if self.temporal_method in method_dic:
            x, weights, timestamps, indices = method_dic[self.temporal_method](x, temporal_length, temporal_weights, temporal_indices)
        else:
...
class FlashVStreamQwen2VisionTransformerPretrainedModel(Qwen2VLPreTrainedModel):
    ...
    def forward_simple_not_merge(self, hidden_states: torch.Tensor, grid_thw: torch.Tensor) -> torch.Tensor:
        ...
        if self.flash_memory.temporal_poolsize > 1:
            ...
            # print(f'In forward_simple_not_merge, hidden_states={hidden_states.shape}, total_grid_thw={total_grid_thw.shape} {total_grid_thw}')
        else:
        ...
        return hidden_states, grid_thw, small_grid_thw
...
class FlashVStreamQwen2VLModel(Qwen2VLForConditionalGeneration):
            ...
            try:
                with self.video_embedding_mem_lock:
                    ...
                    for item in self.video_embedding_memory:
                        cuda_list.append(item.cuda() if hasattr(item, 'cuda') else item)
                    return cuda_list
    ...
    def embed_new_video_clip(self, pixel_values_videos, video_grid_thw, start_idx):
        ...
        def merge_thw(thw_1, thw_2):
            assert thw_1[1:].equal(thw_2[1:]), "Tensors are not equal"
            res = thw_1.clone()
            res[0] += thw_2[0]
            return res
        assert self.use_video_streaming_mode
        logger = logging.getLogger(__name__ + '.embed_new_video_clip')

        pixel_values_videos = pixel_values_videos.type(self.visual.get_dtype())
        pixel_values_videos = pixel_values_videos.to(self.visual.get_device())
        video_grid_thw = video_grid_thw.to(self.visual.get_device())
# self.visual
...
# self.visual.flash_memory
        ...
        tem_x = small_x
        tem_thw = small_thw
        tem_weights = torch.ones(t, dtype=x.dtype, device=x.device)
        tem_timestamp = torch.arange(start_idx + 0, start_idx + t, dtype=x.dtype, device=x.device)
        # read old memory and merge features
        if self.video_embedding_memory is not None and len(self.video_embedding_memory) > 0:
            cuda_list = []
            for item in self.video_embedding_memory:
                cuda_list.append(item.cuda() if hasattr(item, 'cuda') else item)
            old_tem_x, old_tem_thw, old_tem_weights, old_tem_timestamp, _, _, _, old_x, old_thw, old_small_x, old_small_thw, _, _ = cuda_list
            tem_x = torch.cat([old_tem_x, tem_x], dim=0)
            tem_thw = merge_thw(old_tem_thw, tem_thw)
            tem_weights = torch.cat([old_tem_weights, tem_weights], dim=0)
            tem_timestamp = torch.cat([old_tem_timestamp, tem_timestamp], dim=0)
            x = torch.cat([old_x, x], dim=0)
            thw = merge_thw(old_thw, thw)
            small_x = torch.cat([old_small_x, small_x], dim=0)
            small_thw = merge_thw(old_small_thw, small_thw)
        time_3 = time.perf_counter()
        # memory update
        flash = self.visual.flash_memory
        tem_x, tem_thw, tem_weights, tem_timestamp, tem_indices = flash.temporal_compress(tem_x, tem_thw, flash.temporal_length, tem_weights, tem_timestamp)
        time_4 = time.perf_counter()
        tem_positions = tem_timestamp.round().long()
        if flash.spatial_length > 0:
            spa_x, spa_thw, spa_positions = flash.spatial_enhance(
                x=x, 
                small_x=small_x,
                thw=thw, 
                tem_x=tem_x, 
            ...
            spa_thw[0] = 0
            spa_positions = torch.tensor([], device=x.device).long()
        new_x = flash.cat_spa_tem(spa_x=spa_x, tem_x=tem_x)
        flash_memory = new_x.unsqueeze(0)
        time_5 = time.perf_counter()
        video_embeds = self.visual.merger(flash_memory)
        time_6 = time.perf_counter()
        video_embeds = video_embeds.cpu()
        # write memory
        with self.video_embedding_mem_lock:
            self.video_embedding_memory[:] = [
                tem_x.cpu(), tem_thw.cpu(), tem_weights.cpu(), tem_timestamp.cpu(), spa_x.cpu(), spa_thw.cpu(), spa_positions.cpu(),
                x.cpu(), thw.cpu(), small_x.cpu(), small_thw.cpu(), video_embeds, video_embeds.shape
            ]  # Only change content
        # logger.info(f'Write memory, tem_thw={tem_thw}, spa_thw={spa_thw}, thw={thw}, small_thw={small_thw}, video_embeds={video_embeds.shape}')
    ...
    def prepare_realtime_inference(self, position_ids, visual_position_ids):
        ...
        new_position_id = self.visual.flash_memory.calc_am_rope(position_ids[:, 0], visual_position_ids[0], tem_thw, tem_positions, spa_thw, spa_positions)
        return video_embeds, new_position_id.unsqueeze(1)
    ...
    def forward(
        ...
        # print(f'In forward, input_ids={input_ids.shape if input_ids is not None else None}, position_ids = {position_ids.shape if position_ids is not None else None}, video_embeds={video_embeds.shape if video_embeds is not None else None}, video_grid_thw={video_grid_thw.shape if video_grid_thw is not None else None}, visual_position_ids={visual_position_ids.shape if visual_position_ids is not None else None}')
        ...
        if inputs_embeds is None:
            ...
            if self.use_video_streaming_mode:
                ...
                if video_mask.sum() > 0:
                    ...
                    inputs_embeds[video_mask] = video_embeds
                time_1 = time.perf_counter()
                self.user_log_times = [time_0, time_1]
