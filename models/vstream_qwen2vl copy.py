...
class FlashMemory(nn.Module):
    ...
    def temporal_pool(self, x, thw):
        ...
        pile_size = self.temporal_poolsize 
        x = x.reshape(t, h // 2, w // 2, 2 * 2 * xdim)
        # Perform 2x2 avg pooling on dim=1 and dim=2
        # Calculate padding
        pad_h = (h // 2) % 2
        pad_w = (w // 2) % 2
        # Perform repeat padding if necessary
        if pad_h > 0:
            x = torch.cat([x, x[:, -pad_h:, :, :]], dim=1)
        if pad_w > 0:
            x = torch.cat([x, x[:, :, -pad_w:, :]], dim=2)
        new_h = x.shape[1] // 2
        new_w = x.shape[2] // 2
        # Perform 2x2 avg pooling on dim=1 and dim=2
        x = x.permute(0, 3, 1, 2)  # Change shape to [t, 2*2*xdim, h//2, w//2]
        x = F.avg_pool2d(x, kernel_size=2, stride=2)  # Apply 2x2 avg pooling
        x = x.permute(0, 2, 3, 1)  # Change shape back to [t, h//4, w//4, 2*2*xdim]
        x = x.reshape(t, new_h, new_w, 2 * 2 * xdim).reshape(-1, xdim)
        return x
    
    def temporal_compress(self, x, thw):
        # grid_thw is [T/2, H/14, W/14]
        # x.shape is [grid_t x grid_h/2 x grid_w/2 x 2 x 2, 1280]
        t, h, w = thw
        if t <= self.temporal_length:
            return x, torch.ones(t, device=x.device)
        assert h % 2 == 0
        assert w % 2 == 0
        x = x.reshape(t, h // 2 * w // 2 * 2 * 2, x.shape[-1])
        # compress [t, ...] to [temporal_length, ...]
        method_dic = {
            'sample': lambda x, t_len: (x[torch.linspace(0, t - 1, t_len).long()], None, None),
            'merge': merge_feature,
            'drop': drop_feature,
        ...
        if self.temporal_method in method_dic:
            x, weights, step_indices = method_dic[self.temporal_method](x, self.temporal_length)
        else:
            raise ValueError(f"temporal_method should be one of {method_dic.keys()}")
        return x.reshape(-1, x.shape[-1]), weights

    def spatial_enhance(self, x, thw, tem_x, tem_thw, weights):
        t, h, w = thw
        xdim = x.shape[-1]
        x = x.reshape(t, h // 2 * w // 2 * 2 * 2, xdim)
        st, sh, sw = tem_thw
        ...
        if self.spatial_method == 'sample':
            if t > self.spatial_length:
                idx = torch.linspace(0, t - 1, self.spatial_length).long().to(x.device)
                spa_x = x[idx]
            else:
                spa_x = x
        else:
    ...
    def forward(self, x, grid_thw):
        ...
        print(f'In FlashMemory forward, x.shape={x.shape}, grid_thw={grid_thw.shape} {grid_thw} self.spatial_length={self.spatial_length} {self.spatial_method}')
        seqlens = grid_thw.prod(dim=1)
        x_list = torch.split(x, seqlens.tolist())
        new_x = []
        for x, thw in zip(x_list, grid_thw):
            if self.temporal_poolsize > 1:
                small_x = self.temporal_pool(x, thw)
            else:
                small_x = x
            small_compressed_x, weights = self.temporal_compress(small_x, get_pooled_thw(thw, self.temporal_poolsize))
            if self.spatial_length > 0:
                x = self.spatial_enhance(x, thw, small_compressed_x, get_pooled_compressed_thw(thw, self.temporal_poolsize, self.temporal_length), weights)
            else:
...
class FlashVStreamQwen2VisionTransformerPretrainedModel(Qwen2VLPreTrainedModel):
    ...
    def __init__(self, config) -> None:
        ...
        if getattr(config, 'flash_memory_config', None) is None:
            ...
            config.flash_memory_config = {
                'flash_memory_temporal_length': 10000,
                'flash_memory_temporal_method': 'sample',
                'flash_memory_temporal_poolsize': 1,
                'flash_memory_spatial_length': 0,
                'flash_memory_spatial_method': 'sample',
    ...
    def forward(self, hidden_states: torch.Tensor, grid_thw: torch.Tensor) -> torch.Tensor:
        ...
        hidden_states = self.flash_memory(hidden_states, grid_thw)
...
class FlashVStreamQwen2VLConfig(Qwen2VLConfig):
    ...
    def set_flash_memory_config(
        ...
        self.vision_config.flash_memory_config = {
    ...
    def forward(
        ...
        if inputs_embeds is None:
            ...
            if pixel_values_videos is not None:
                ...
                video_embeds = self.visual(pixel_values_videos, grid_thw=video_grid_thw).to(inputs_embeds.device)  # [N, dim]
