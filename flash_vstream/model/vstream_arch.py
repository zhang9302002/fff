...
class VStreamMetaModel:
    ...
    def __init__(self, config):
        ...
        if getattr(config, 'mm_hidden_size', False):
            self.mm_input_dim = config.mm_hidden_size
        else:
            config.mm_hidden_size = 1024
            config.mm_projector_lr = None,
            config.mm_projector_type = "mlp2x_gelu",
            config.mm_resampler_type = None,
            config.mm_use_4_vision_tokens = False,
            config.mm_use_im_patch_token = False,
            config.mm_use_im_start_end = False,
            config.mm_vision_select_feature = "patch",
            config.mm_vision_select_layer = -2,
            config.mm_vision_tower = "./ckpt/clip-vit-large-patch14",
