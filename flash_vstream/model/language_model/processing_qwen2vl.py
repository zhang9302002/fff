...
class Qwen2VLProcessor(ProcessorMixin):
    ...
    def __call__(
        ...
        if videos is not None:
            ...
            new_video_grid_thw[:, 0] = torch.minimum(new_video_grid_thw[:, 0], torch.tensor(T_max_frames))
            for i in range(len(text)):
                while "<|video_pad|>" in text[i]:
                    text[i] = text[i].replace(
                        "<|video_pad|>", "<|placeholder|>" * (new_video_grid_thw[index].prod() // merge_length), 1
                    )
                    index += 1
                text[i] = text[i].replace("<|placeholder|>", "<|video_pad|>")
            videos_inputs["new_video_grid_thw"] = new_video_grid_thw
        else:
            videos_inputs = {}
            video_grid_thw = None
